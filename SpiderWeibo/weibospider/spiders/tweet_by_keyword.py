...
class TweetSpiderByKeyword(Spider):
    ...
    def __init__(self, keywords:list, st_y: int, st_mth: int, st_day: int, end_y: int, end_mth: int, end_day: int):
        super(TweetSpiderByKeyword, self).__init__()
    ...
    def start_requests(self):
        ...
        # 这里的时间可替换成实际需要的时间段
        start_time = datetime.datetime(year=self.st_y, month=self.st_mth, day=self.st_day, hour=0)
        end_time = datetime.datetime(year=self.end_y, month=self.end_mth, day=self.end_day, hour=23)
